...
When(/^Click on Search icon$/) do
  sleep(10)
  @page.click_element '#header-search-submit'
end

Then(/^Displaying Filtered Restaurant list$/) do
  sleep(10)
  @page.click_element('.all-filters-toggle')
end
...
Then(/^Compare Results of restaurants and restaurants pizza$/) do
  sleep 10 
  ## results are not uniq so it can be fail
...
When(/^Click on neighbourhood and price$/) do
  sleep 10
  @page.click_element('#wrap > div.main-content-wrap.main-content-wrap--full > div.top-shelf.top-shelf-grey > div > div.clearfix.layout-block.layout-full.search-page-top > div > div.filter-panel > div.filter-panel_filters > div.filter-set.price-filters > ul > li:nth-child(2) > label > span') &&
  @page.click_element('#wrap > div.main-content-wrap.main-content-wrap--full > div.top-shelf.top-shelf-grey > div > div.clearfix.layout-block.layout-full.search-page-top > div > div.filter-panel > div.filter-panel_filters > div.filter-set.place-filters > ul.main > li:nth-child(2) > label > span')
end
...
# expect(@page.get_element_star_rating("#super-container > div > div.clearfix.layout-block.layout-a.scroll-map-container.search-results-block > div.column.column-alpha > div > div.search-results-content > ul:nth-child(2) > li:nth-child(#{index}) > div.search-result > div.biz-listing-large > div.main-attributes > div.media-block.media-block--12 > div.media-story > div.biz-rating > div.i-stars")).should include("can't be blank")
